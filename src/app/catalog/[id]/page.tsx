...
// В реальном проекте мы бы делали fetch по id. 
// Для примера используем тот же моковый массив и находим товар по id.
const CATALOG_DB = [
  ...
  // ... (Остальные товары из предыдущего файла)
];
...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  // Разворачиваем params через хук use
  const { id } = use(params);
  const productId = parseInt(id);

  // Находим товар (если не найден, берем первый для демо)
  const product = CATALOG_DB.find(p => p.id === productId) || CATALOG_DB[0];
        ...
        <div className="w-full max-w-6xl px-6">
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
            ...
            <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden rounded-3xl bg-zinc-900 shadow-2xl animate-[fadeUp_0.5s_ease-out]">
              ...
              <div className="absolute top-6 left-6 font-mono text-xs font-bold tracking-widest text-orange-500/50">
                DATA_ID: {product.code}
              </div>
              
              <div className="relative z-10 text-[12rem] md:text-[16rem] drop-shadow-[0_0_40px_rgba(249,115,22,0.4)] animate-[float_6s_ease-in-out_infinite]">
                {product.img}
              </div>
            </div>

            {/* Правая колонка: Информация и конфигуратор */}
            <div className="flex flex-col justify-center animate-[fadeUp_0.7s_ease-out]">
                ...
                <div className="flex gap-4">
                  {product.sizes.map((size: string) => (
                    <button
                      ...
                      className={`relative flex h-16 flex-1 flex-col items-center justify-center rounded-xl border-2 transition-all ${
                        selectedSize === size 
                    ...
                    >
                      ...
                      {SIZE_MULTIPLIER[size] > 0 && (
                        <span className="text-xs text-zinc-400">+{SIZE_MULTIPLIER[size]} ₸</span>
                      )}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {MILK_OPTIONS.map((milk) => (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-full border px-5 py-3 font-semibold transition-all ${
              ...
              <div className="mt-auto flex items-center justify-between rounded-3xl bg-white p-4 shadow-lg ring-1 ring-zinc-100">
                ...
                <button 
                  ...
                  disabled={isAdding}
                  className={`relative flex h-16 w-48 items-center justify-center overflow-hidden rounded-2xl font-bold text-white transition-all ${
                    isAdding ? 'bg-green-500' : 'bg-zinc-900 hover:bg-orange-500'
                  }`}
                >
                  <span className={`transition-transform duration-300 ${isAdding ? '-translate-y-12' : 'translate-y-0'}`}>
                    В корзину
                  </span>
